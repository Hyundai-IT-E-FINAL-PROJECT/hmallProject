...
;(function (core, $, undefined) {
    ...
    core.addon('util', /** @lends UI.util */{
        ...
        cssLiteralToObject: function (str) {
            var strValue = str, arr = strValue.split(';'), i = arr.length - 1, obj = {}, objArr, objFirst, objLast,
                objName, objProp, arrFirst, arrLast, firstStr, lastStr;
            for (; i >= 0; i--) {
                if (arr[i] !== "") {
            ...
            data.forEach(function (element) {
                ...
                if (!category.CATEGORY_REF) {
                    $(".category-list ul").append(
                        `<li data-catecode="${category.CATEGORY_SEQ}"><a href="/product/all?category_seq=${category.CATEGORY_SEQ}">${category.CATEGORY_NAME}</a></li>`
                    )
...
$(function fn_searchAdKeywordDefault() {
        $.ajax({
            url: "http://localhost:8080/product/main"
            , type: "get"
            , dataType: "json"
            , async: false
                        ...
                        <li class="pdthumb">
                            <a href="product/detail?product_seq=${product.PRODUCT_SEQ}" onclick="gaTagging(this, '', '', '');" class="item-box" ga-custom-position="마이템_추천" ga-custom-id="${product.PRODUCT_NAME}" ga-custom-name="메인_홈쇼핑탭" ga-custom-creative="BDA_bdTrkArea=34&amp;bdTrkAlgrth=11_자동" tabindex="0">
                                ...
                                <div class="figcaption">
                                    <div class="pdname" aria-label="${product.PRODUCT_NAME}">
                                        ${product.PRODUCT_NAME}
                                    </div>
                                    <div class="pdprice">
                                        <span class="rateprice" aria-label="할인율이 적용된 가격">
                                            <p class="discount" aria-label="할인가"><em>${product.PRODUCT_COST.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</em>원</p>
                                        </span>
                                    </div>

                                </div>
                            </a>
                            <a href="/p/pda/itemPtc.do?slitmCd=2101560521&amp;sectId=&amp;bdTrkArea=34&amp;bdTrkAlgrth=11&amp;bdTrkOpt=A" class="hoverview" target="_blank" onclick="gaTagging(this, '', '', '');" ga-custom-position="마이템_추천" ga-custom-id="2101560521_SPC삼립 필라프 5종(총15팩) + 비프 함박스테이크 2종(총15장)" ga-custom-creative="BDA_bdTrkArea=34&amp;bdTrkAlgrth=11_자동" tabindex="0"><i class="icon"></i>새창열기</a>
                            <!--// 새창열기 a태그로 변경 후 위치이동 -->
                        </li>
                        `
                    )
